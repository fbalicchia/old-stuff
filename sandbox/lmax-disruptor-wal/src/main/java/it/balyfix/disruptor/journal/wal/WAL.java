...
public class WAL<T extends Writable> implements Closeable {
  ...
  public WAL(File path, Class<T> clazz) throws IOException {
  ...
  private void roll() throws IOException {
    try {
      ...
      dataFileWALWriter = new WALDataFile.Writer<>(dataWalFileName);
      sequenceIDWALWriter = new WALDataFile.Writer<>(seqWalFileName);
      rollRequired = false;
    } finally {
  ...
  public WALReplayResult<T> replay() throws IOException {
    ...
    readFiles(sequenceIDPath, input ->
    {
